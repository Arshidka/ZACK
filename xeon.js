...
async function startXeonBotInc() {
    const XeonBotInc = XeonBotIncConnect({
        ...
        browser: ['Subscribe A R S H I D','Safari','1.0.0'],
        auth: state
    })
    ...
    // Get Profile Picture Group
       ...
       XeonBotInc.send5ButImg(pea[0].id, `「 Group Settings Changed 」\n\nThe Group Has Been Closed By Admin, Now Only Admin Can Send Messages !`, `${botname}`,lolZack, [])
       } else if(pea[0].announce == false) {
       XeonBotInc.send5ButImg(pea[0].id, `「 Group Settings Changed 」\n\nThe Group Has Been Opened By Admin, Now Participants Can Send Messages !`, `${botname}`,lolZack, [])
       } else if (pea[0].restrict == true) {
       XeonBotInc.send5ButImg(pea[0].id, `「 Group Settings Changed 」\n\nGroup Info Has Been Restricted, Now Only Admin Can Edit Group Info !`, `${botname}`,lolZack, [])
       } else if (pea[0].restrict == false) {
       XeonBotInc.send5ButImg(pea[0].id, `「 Group Settings Changed 」\n\nGroup Info Has Been Opened, Now Participants Can Edit Group Info !`, `${botname}`,lolZack, [])
       } else {
       XeonBotInc.send5ButImg(pea[0].id, `「 Group Settings Changed 」\n\nGroup Subject Has Been Changed To *${pea[0].subject}*`, `${botname}`,lolZack, [])
     }
    })
...
└┬❖ 「 @${xeonName.split("@")[0]}  」
   ...
   //you fail to put, i sue you for sure!
let buttons = [
{buttonId: `wkwkwk`, buttonText: {displayText: 'O K B Y E 🥀'}, type: 1}
]
let buttonMessage = {
document: fs.readFileSync('./XeonMedia/theme/cheems.xlsx'),
mimetype: docs,
